            ...
            if (pageIndex != 0) {
                ...
                if (StringUtils.isNotBlank(searchValue)) {
                    if (searchValue.matches(matche)) {
                        querySql="select * from eipv6 where is_delete='0' and project_id= '"+projectId+"'"+" and id="+searchValue;
                        ...
                        querySql="select * from eipv6 where is_delete='0' and project_id= '"+projectId+"'"+" and sbw_name="+searchValue;
                        page = ListFilterUtil.filterPageDataBySql(entityManager, querySql, pageable, Sbw.class);
                        //page = sbwDaoService.findByIsDeleteAndSbwName(projectId, 0, searchValue, pageable);
                    }
                } else {
                    querySql="select * from eipv6 where is_delete='0' and project_id= '"+projectId+"'";
                     page = ListFilterUtil.filterPageDataBySql(entityManager, querySql, pageable, Sbw.class);
